...
public:
    ...
    void Reverse()
    {
        ...
        while (ptr != NULL) 
        { 
            arr[veces] = ptr -> get_val(); 
            ptr = ptr -> get_next();

            veces++;
        } 
        ptr = head; //apunta a final de la lista (arriba)
        int veces2 = length(); //contador Reverse...

        while (ptr != NULL) 
        { 
            ptr ->set_val(arr[veces2 -1]);
            ptr = ptr -> get_next();

            veces2--; //entra en reversa al arreglo y queda...
        } 
    }
